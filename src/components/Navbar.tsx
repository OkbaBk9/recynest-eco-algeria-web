    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'glass-nav py-2 px-6 md:px-12' : 'bg-transparent py-4 px-6 md:px-12'}`}>
      <div className="container mx-auto flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center space-x-8">
          {['How It Works', 'Drop Points', 'Marketplace', 'Rewards', 'Our Mission'].map((item, index) => (
            <motion.a
              key={item}
              ...
              className="text-foreground hover:text-recynest-dark transition-colors relative group"
            >
              {item}
        ...
        <div className="hidden lg:flex items-center space-x-4">
          ...
          >
            ...
            <input
              type="text"
              placeholder="Search..."
              className="pl-10 pr-4 py-2 rounded-full border border-border glass-morphism text-sm focus:outline-none focus:ring-2 focus:ring-recynest-medium"
            />
          </motion.div>
          <motion.div
            initial={{ opacity: 0, scale: 0.9 }}
      ...
      {isMenuOpen && (
        ...
        >
          <div className="flex flex-col space-y-4 p-4">
            {['How It Works', 'Drop Points', 'Marketplace', 'Rewards', 'Our Mission'].map((item) => (
              <a 
                key={item}
                href={`#${item.toLowerCase().replace(/\s+/g, '-')}`} 
                className="text-foreground hover:text-recynest-dark transition-colors p-2 rounded-lg hover:bg-recynest-light/30"
